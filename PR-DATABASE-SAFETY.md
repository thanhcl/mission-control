...
- Stores backups in a `db-backups/` subdirectory next to the database file
- Keeps the last 5 backups, automatically removes older ones
- If backup creation fails, **aborts the migration run entirely** and throws an error — protecting data takes priority over applying schema changes
...
**What it does:**
- Before the destructive `DELETE FROM` loop, checks `SELECT COUNT(*) FROM tasks`
- If the database has any existing tasks (real data), **skips all destructive deletes** and logs a prominent warning
...
The guard makes the destructive behavior conditional: it only runs on databases that have no tasks (i.e., genuinely fresh installs), matching the original intent.

---

## Testing

- App boots cleanly: `npm run dev` ✅
- API responds correctly: `GET /api/agents` returns data ✅  
